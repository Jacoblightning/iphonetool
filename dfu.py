...
async def main(dev: usb.core.Device, parser: argparse.ArgumentParser, subparsers: argparse._SubParsersAction):
    ...
    if pwned_device:
        subparsers.add_parser("demote", help="Demote this device to a development device")

        boot_parser = subparsers.add_parser("boot", help="Low-level device exploit booting. You probably want \"linux\"")
        boot_subparsers = boot_parser.add_subparsers(dest="boot_action")

        boot_raw_parser = boot_subparsers.add_parser("raw", help="Boot a raw iBoot file")
        boot_raw_parser.add_argument("iBoot", type=pathlib.Path, help="The raw iBoot to boot")
        
        boot_remote_parser = boot_subparsers.add_parser("remote", help="Remote boot m1n1, linux, etc.")
        boot_remote_parser.add_argument("-1", "--m1n1", type=pathlib.Path, help="m1n1-idevice.macho + any addons (linux, dtbs, etc)", required=True)
        boot_remote_parser.add_argument("-m", "--monitor", type=pathlib.Path, help="m1n1 monitor stub")

        # High-level linux booter
        linux_parser = subparsers.add_parser("linux", help="Boot linux on device")
        linux_parser.add_argument("-1", "--m1n1", type=pathlib.Path, help="Path to m1n1-idevice.macho", required=True)
        linux_parser_dtb_group = linux_parser.add_mutually_exclusive_group(required=True)
        linux_parser_dtb_group.add_argument("-d", "--dtb", type=pathlib.Path, help="Path to devicetree for device")
        linux_parser_dtb_group.add_argument("-D", "--dtbs", type=pathlib.Path, help="Path to directory of devicetrees")
        linux_parser.add_argument("-k", "--kernel", type=pathlib.Path, help="Path to kernel", required=True)
        linux_parser.add_argument("-c", "--commandline", help="Linux commandline to boot")
        linux_parser.add_argument("-i", "--initramfs", type=pathlib.Path, help="Linux initramfs to boot")
        linux_parser.add_argument("-m", "--monitor", type=pathlib.Path, help="m1n1 monitor stub")
...
def send_usbliter8_command(dev: usb.core.Device, command: Usbliter8Command, data: Any, timeout: int) -> None:
...
def usbliter8_download(dev: usb.core.Device, data: bytes) -> True:
    ...
    while left:
        ...
        send_usbliter8_command(dev, Usbliter8Command.DFU_DNLOAD, data[offset:offset+current_block_length], 1000)
